...
export class ReplayJobComponent implements OnInit {
  ...
  addEndpointMapping = (): void => {
    this.getTargetEndpointMapping().subscribe(endpointMapping => {
      const replayMapping = Object.assign({}, endpointMapping, {
  ...
  createReplayJob = (): RecrepReplayJob => {
    const replayJob = Object.assign(<RecrepReplayJob>{}, {
    ...
    // if (this.replayJobState.scheduled) {
      // replayJob.timestampStart = moment(this.replayJobState.startDate + ' '
      //  + this.replayJobState.startTime, 'DD.MM.YYYY HH:mm').valueOf();
    // } else {
      // replayJob.timestampStart = moment().add(1, 'seconds').valueOf();
    // }
  ...
  validateReplayJob = (): any => {
    return (group: FormGroup): {[key: string]: any} => {
      ...
      const noEndpointMapping = this.replayJobState.endpointMappings.length === 0;
